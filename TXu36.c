...
void main(int argc, char *argv[])
{
  ...
  if (argc > 1 && (argv[1][0] == '/' || argv[1][0] == '!')) // 有指定書號
  {
    ...
    {
      strcpy(inDir, "ch??.txt");    // 搜尋所有 "chxx.txt" 的檔名
      Process();
    }
  ...
  {
    for (idx = argc - 1; idx > 0; idx--) // 從命令列尾端往回找到指定輸出路徑的參數
      if (argv[idx][0] != '/')
        break;
...
void Process()
{
  ...
  done = ((hFile = _findfirst(inDir, &fs)) == -1L);
  while (!done)
  {
  ...
  skip:
    done = (_findnext(hFile, &fs) == -1L); // 找下一個檔
  }
  ...
  if (sys) // 如果要包含子資料夾
  {
    ...
    // printf("\ninpth dir %s", inPath);
    // 只搜尋資料夾
    done = ((hFile = _findfirst(inPath, &fs2)) == -1L);
    while (!done)
    {

      // 剔除代表目前資料夾的 '.'
      if (fs2.name[0] != '.' && (fs2.attrib & _A_SUBDIR))
